...
```
The true variance for data generating parameter $\theta$ is $1.003\mathrm{e}{-5}$, and we can then compare the performance of two functions `bootsvar` and `deltavar` for estimating $\text{var}(\theta)$.
```{r, eval=FALSE}
bootsvar(data = observ, time = times, ode.model = ode.model, 
                       par.initial = 0.3, par.names = 'theta',state.names = 'X',
